  ...
  useEffect(() => {
    const fetchAssets = async () => {
      ...
      try {
        ...
        setError(
          error.response ? error.response.data : "Erro ao buscar assets"
        );
      } finally {
        setLoading(false);
      }
          ...
          <ol>
            {assets.map((asset, index) => (
              <li key={index} className="asset-item">
                <div>
                  <strong>Data:</strong> {asset.data}
                </div>
                <div>
                  <strong>Sigla:</strong> {asset.sigla}
                </div>
                <div>
                  <strong>Setor:</strong> {asset.setor}
                </div>
                <div>
                  <strong>Preço:</strong> {asset.preco}
                </div>
                <div>
                  <strong>Cotas:</strong> {asset.cotas}
                </div>
              </li>
            ))}
          </ol>
